function PlotViscosity(Ttime_ind,X,ft,Ttime,refFactor,cbarlim,alph,Wi)
...
gamma = (2*(eu11.*eu11+eu12.*eu12+eu21.*eu21+eu22.*eu22)).^(1/2);

alpha = alph;
De    = Wi;

chi2 = ((1+16*alpha*(1-alpha)*(De*gamma).^2).^(1/2)-1)./(8*alpha*(1-alpha)*(De*gamma).^2);
f = (1-sqrt(chi2))./(1+(1-2*alpha)*sqrt(chi2));
vis(:,:) = ((1-f).^2)./(1+(1-2*alpha)*f);

hold on;

pcol = pcolor(xvf,yvf,vis);
