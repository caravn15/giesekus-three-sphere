function detJ = CalculateSinhJacobian(y0t,x2D,model)
...
detJ = zeros(length(x2D),model.mesh.nelem);
for e=1:model.mesh.nelem
    ...
    dx = (ep*m(1)*cosh(m(1)*asinh(X)))./(sqrt(X.^2+1));
    dy = (ep*m(2)*cosh(m(2)*asinh(Y)))./(sqrt(Y.^2+1));

    detJSinh = dx.*dy;
    detJRef  = (xlimt(2)-xlimt(1))*(ylimt(2)-ylimt(1))/4;
    detJ(:,e)  = detJSinh*detJRef;
end
